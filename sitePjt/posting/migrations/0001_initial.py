# Generated by Django 2.1.5 on 2020-03-23 03:39

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion
import uuid


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Comment',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('contentType', models.CharField(choices=[('text', 'text/plain'), ('md', 'text/markdown')], default='text', max_length=4)),
                ('comment', models.CharField(max_length=200)),
                ('published', models.DateTimeField(auto_now_add=True, verbose_name='date posted')),
                ('author', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
        migrations.CreateModel(
            name='Post',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('title', models.CharField(max_length=50)),
                ('contentType', models.CharField(choices=[('png', 'image/png;base64'), ('md', 'text/markdown'), ('text', 'text/plain'), ('app', 'application/base64'), ('jpeg', 'image/jpeg;base64')], default='text', max_length=4)),
                ('origin', models.CharField(blank=True, max_length=50)),
                ('source', models.CharField(blank=True, max_length=50)),
                ('content', models.CharField(max_length=200)),
                ('categories', models.CharField(blank=True, max_length=200)),
                ('unlisted', models.BooleanField(choices=[(True, 'True'), (False, 'False')], default=False)),
                ('published', models.DateTimeField(auto_now_add=True, verbose_name='date posted')),
                ('visibility', models.CharField(choices=[('PUBLIC', 'Public'), ('PRIVATE', 'Prviate to self'), ('FRIENDS', 'Private to friends'), ('FOAF', 'Private to friends of friends'), ('SERVERONLY', 'Private to local friends')], default='PUBLIC', max_length=10)),
                ('author', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
        migrations.AddField(
            model_name='comment',
            name='post',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='posting.Post'),
        ),
    ]
